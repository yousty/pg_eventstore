$(function(){
    ...
    let initEventTypeFilterAutocomplete = function($filter) {
        let $eventTypeSelect = $filter.find('select.type-filter');
        let $eventMarkersSelect = $filter.find('select.event-markers-filter');
        ...
        $eventMarkersSelect.select2({
            ajax: {
                url: $eventMarkersSelect.data('url'),
                processResults: function(data, params){
                    params.starting_id = data.pagination.starting_id;
                    data.results.forEach(function(contextAttrs){
                        contextAttrs.id = contextAttrs.name;
                        contextAttrs.text = contextAttrs.name;
                    });
                    return data;
                },
            },
    ...
    let initMarkersFilterAutocomplete = function($filter) {
        let $markersSelect = $filter.find('select.markers-filter');
        $markersSelect.select2({
            ajax: {
                url: $markersSelect.data('url'),
                processResults: function(data, params){
                    params.starting_id = data.pagination.starting_id;
                    data.results.forEach(function(contextAttrs){
                        contextAttrs.id = contextAttrs.name;
    ...
    // Markers filter template
    let markersFilterTmpl = $('#markers-filter-tmpl').text();
    ...
    // "Add markers filter" button
    $filtersForm.on('click', '.add-markers-filter', function(){
        $filtersForm.find('.markers-filter').append(markersFilterTmpl);
        initMarkersFilterAutocomplete($filtersForm.find('div.markers-filter').children().last());
    });
    ...
    // Init select2 for markers filter which was initially rendered
    $filtersForm.find('.markers-filter').children().each(function(){
        initMarkersFilterAutocomplete($(this));
    });
