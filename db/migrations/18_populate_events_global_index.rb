...
partitions = PgEventstore.connection(:_eventstore_db_connection).with do |conn|
  conn.exec('select * from partitions where stream_name is not null')
end
...
threads = CONCURRENCY.times.map do |t|
  Thread.new do
    partitions.each_value do |stream_name_parts|
        ...
        event_type_parts.each_value do |event_type_part|
          ...
          loop do
            ...
            stream_builders = events.map do |event|
              ...
              sql_builder.where('stream_id = ? and partition_id = ?', event['stream_id'], stream_name_part['id'])
            end
            ...
            values = events.map do |event|
              ...
              "(#{event['global_position']}, #{event_type_part['id']}, '#{stream_idx_id}')"
            end.join(',')
