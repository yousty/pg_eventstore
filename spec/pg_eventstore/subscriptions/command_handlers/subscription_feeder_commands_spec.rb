...
RSpec.describe PgEventstore::CommandHandlers::SubscriptionFeederCommands do
  ...
  describe '#process' do
    ...
    context 'when there is a "StopAll" command' do
      let!(:command) do
        command_queries.create(subscriptions_set_id:, command_name: 'StopAll', data: {})
      end
      ...
      it 'deletes it' do
        expect { subject }.to change {
          command_queries.find_by(subscriptions_set_id:, command_name: 'StopAll')
        }.to(nil)
      end
    end
    ...
    context 'when there is a "StartAll" command' do
      let!(:command) do
        command_queries.create(subscriptions_set_id:, command_name: 'StartAll', data: {})
      end
      ...
      it 'deletes it' do
        expect { subject }.to change {
          command_queries.find_by(subscriptions_set_id:, command_name: 'StartAll')
        }.to(nil)
      end
    end
    ...
    context 'when there is a "Restore" command' do
      let!(:command) do
        command_queries.create(subscriptions_set_id:, command_name: 'Restore', data: {})
      end
      ...
      it 'deletes it' do
        expect { subject }.to change {
          command_queries.find_by(subscriptions_set_id:, command_name: 'Restore')
        }.to(nil)
      end
    end
    ...
    context 'when there is a "Stop" command' do
      let!(:command) do
        command_queries.create(subscriptions_set_id:, command_name: 'Stop', data: {})
      end
      ...
      it 'deletes it' do
        expect { subject }.to change {
          command_queries.find_by(subscriptions_set_id:, command_name: 'Stop')
        }.to(nil)
      end
    end
    ...
    context 'when there is an unhandled command' do
      let!(:command) do
        command_queries.create(subscriptions_set_id:, command_name: 'FooCmd', data: {})
      end

      it 'deletes it' do
        expect { subject }.to change {
          command_queries.find_by(subscriptions_set_id:, command_name: 'FooCmd')
        }.to(nil)
      end
    end
