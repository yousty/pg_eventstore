  ...
  describe '#call' do
    ...
    context 'when there are some events' do
      ...
      it 'runs :process callbacks for first event' do
        subject
        aggregate_failures do
          expect(before_process_action).to have_received(:call).with(indexes.first.subscription_position, 1)
          expect(after_process_action).to have_received(:call).with(indexes.first.subscription_position, 1)
        end
      end
      it 'does not run :process callbacks for second event' do
        subject
        aggregate_failures do
          expect(before_process_action).not_to have_received(:call).with(indexes.last.subscription_position)
          expect(after_process_action).not_to have_received(:call).with(indexes.last.subscription_position)
        end
      end
