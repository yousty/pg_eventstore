...
- `id` - String(UUID, optional, not `nil`). If not provided - the value will be autogenerated on the database side using
  `gen_random_uuid()`. Application-specific identifier, does not affect on anything internally. Uniqueness is not
  guaranteed explicitly, but you can rely on `gen_random_uuid()` as a source of unique values.
